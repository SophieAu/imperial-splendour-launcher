...
type APIMap = { play: APII; website: APII; downloadPage: APII; exit: APII };
...
export const callAPI = (API: APIType) => async (key: EndpointKeys): Promise<void> => {
  const apiMap: APIMap = {
    ...
    downloadPage: { callEndpoint: API.GoToDownloadPage, expectedErr: Errors.DOWNLOAD_PAGE },
...
const mapError = ({ message }: Error, expectedErr?: Errors): string => {
  ...
  else if (message === Errors.DOWNLOAD_PAGE) return apiErrors.downloadPage;
