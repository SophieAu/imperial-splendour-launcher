...
func TestUninstall(t *testing.T) {
	...
	t.Run("In strict Mode cannot deactivate Imperial Splendour", func(t *testing.T) {
		api, sysHandler := test.VariableBefore("2.0", true, "test")
	...
	t.Run("In non-strict Mode can deactivate Imperial Splendour but has silent errors", func(t *testing.T) {
		api, sysHandler := test.VariableBefore("2.0", true, "test")
		sysHandler.On("ReadFile", "./IS_Files/IS_FileList.txt").Return([]byte("merp.pack\nderp.tga"), nil)
		sysHandler.On("WriteFile", "./IS_Files/IS_Info.json", mock.Anything).Return(errors.New("StatusUpdateError")).Once()
		sysHandler.On("MoveFile", mock.Anything, mock.Anything).Return(nil)
